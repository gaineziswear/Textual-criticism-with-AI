export async function proxyResearch(req: Request): Promise<Response> {
  const incoming = new URL(req.url);
  const route = incoming.searchParams.get("__route") || incoming.pathname;
  ...
  try {
    ...
    return Response.json({ error: "Research service unavailable.", detail: message }, { status: 502 });
  }
}
